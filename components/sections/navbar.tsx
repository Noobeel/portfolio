...
export default function Navbar() {
        ...
        window.onscroll = () => {
            ...
            sections.forEach((section) => {
                ...
                if (lastScrollPos > topOfScreen) {
                    if (
                        topOfScreen > sectionTop &&
                        topOfScreen < sectionBottom - triggerHeight
                    ) {
                        if (section.id === "hero") {
                            window.history.pushState({}, "", "/");
                            document
                                .getElementById("tab")
                                ?.classList.remove("visible-tab");
                            return;
                        }
                    ...
                    if (
                        bottomOfScreen > sectionTop + triggerHeight &&
                        bottomOfScreen < sectionBottom
                    ) {
                        if (section.id === "hero") {
                            window.history.pushState({}, "", "/");
                            document
    ...
    return (
        ...
        >
            <ul
                id="nav-list"
                className="relative list-none flex justify-center items-center"
            >
                ...
                <div
                    id="tab"
                    className="absolute z-10 opacity-0 rounded-full backdrop-blur-sm bg-active transition-all ease-in-out"
                />
